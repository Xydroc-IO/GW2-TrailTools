	...
	const PathingTrails::WorldSnippet* guideOrNull)
{
	...
	FLOAT prevBf[4]{};
	UINT prevSm = 0;
	gCtx->OMGetBlendState(&prevBlend, prevBf, &prevSm);
	ID3D11DepthStencilState* prevDss = nullptr;
	UINT prevStencil = 0;
	gCtx->OMGetDepthStencilState(&prevDss, &prevStencil);
	ID3D11RasterizerState* prevRs = nullptr;
	gCtx->RSGetState(&prevRs);
	ID3D11InputLayout* prevIl = nullptr;
	gCtx->IAGetInputLayout(&prevIl);
	D3D11_PRIMITIVE_TOPOLOGY prevTopo = D3D11_PRIMITIVE_TOPOLOGY_UNDEFINED;
	gCtx->IAGetPrimitiveTopology(&prevTopo);
	ID3D11Buffer* prevVb = nullptr;
	UINT prevStride = 0, prevOff = 0;
	gCtx->IAGetVertexBuffers(0, 1, &prevVb, &prevStride, &prevOff);
	ID3D11VertexShader* prevVs = nullptr;
	UINT vsN = 0;
	gCtx->VSGetShader(&prevVs, nullptr, &vsN);
	ID3D11PixelShader* prevPs = nullptr;
	UINT psN = 0;
	gCtx->PSGetShader(&prevPs, nullptr, &psN);
	ID3D11Buffer* prevVsCb = nullptr;
	gCtx->VSGetConstantBuffers(0, 1, &prevVsCb);
	ID3D11Buffer* prevPsCb = nullptr;
	gCtx->PSGetConstantBuffers(0, 1, &prevPsCb);
	ID3D11SamplerState* prevSamp = nullptr;
	gCtx->PSGetSamplers(0, 1, &prevSamp);
	ID3D11ShaderResourceView* prevSrv = nullptr;
	gCtx->PSGetShaderResources(0, 1, &prevSrv);
	...
	gCtx->OMSetBlendState(prevBlend, prevBf, prevSm);
	gCtx->OMSetDepthStencilState(prevDss, prevStencil);
	gCtx->RSSetState(prevRs);
	gCtx->IASetInputLayout(prevIl);
	gCtx->IASetPrimitiveTopology(prevTopo);
	gCtx->IASetVertexBuffers(0, 1, &prevVb, &prevStride, &prevOff);
	gCtx->VSSetShader(prevVs, nullptr, 0);
	gCtx->PSSetShader(prevPs, nullptr, 0);
	gCtx->VSSetConstantBuffers(0, 1, &prevVsCb);
	gCtx->PSSetConstantBuffers(0, 1, &prevPsCb);
	gCtx->PSSetSamplers(0, 1, &prevSamp);
	gCtx->PSSetShaderResources(0, 1, &prevSrv);
