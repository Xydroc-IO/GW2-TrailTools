...
namespace
{
	...
	{
		return std::string(static_cast<size_t>(n * 3), ' ');
	}
	...
	{
		std::string pad = Indent(indentTabs);
		...
		if (std::fabs(p.mapDisplaySize - 20.f) > 0.01f)
			os << " mapDisplaySize=\"" << p.mapDisplaySize << "\"";
		if (std::fabs(p.minSize - 5.f) > 0.01f)
			os << " minSize=\"" << p.minSize << "\"";
		if (std::fabs(p.maxSize - 2048.f) > 0.5f)
			os << " maxSize=\"" << p.maxSize << "\"";
		if (!p.minimapVisible)
			os << " miniMapVisibility=\"0\"";
		if (!p.inGameVisible)
			os << " inGameVisibility=\"0\"";
		if (p.achievementId >= 0)
			os << " achievementId=\"" << p.achievementId << "\"";
		if (p.achievementBit >= 0)
			os << " achievementBit=\"" << p.achievementBit << "\"";
		if (!p.festival.empty())
			os << " festival=\"" << EscAttr(p.festival) << "\"";
		if (p.profession != 0)
			os << " profession=\"" << p.profession << "\"";
		if (p.race != 0)
			os << " race=\"" << p.race << "\"";
		if (p.mount != 0)
			os << " mount=\"" << p.mount << "\"";
		if (!p.toggleCategory.empty())
			os << " toggleCategory=\"" << EscAttr(p.toggleCategory) << "\"";
		if (!p.tipName.empty())
	...
	void EmitTrailTag(std::ostringstream& os, const TrailToolsDetail::DraftTrail& t,
		const std::string& pad)
	{
		os << pad << "<Trail type=\"" << EscAttr(t.type)
			<< "\" trailData=\"" << EscAttr(t.fileRel) << "\"";
		if (!t.texture.empty())
			os << " texture=\"" << EscAttr(t.texture) << "\"";
		if (t.animSpeed > 0.f && std::fabs(t.animSpeed - 1.f) > 0.001f)
			os << " animSpeed=\"" << t.animSpeed << "\"";
		if (t.alpha > 0.f && t.alpha < 0.999f)
			os << " alpha=\"" << t.alpha << "\"";
		if (t.fadeNear >= 0.f)
			os << " fadeNear=\"" << t.fadeNear << "\"";
		if (t.fadeFar >= 0.f)
			os << " fadeFar=\"" << t.fadeFar << "\"";
		if (t.trailScale > 0.f && std::fabs(t.trailScale - 1.f) > 0.001f)
			os << " trailScale=\"" << t.trailScale << "\"";
		os << "/>\n";
	}
	...
	void EmitTrailLines(std::ostringstream& os, const TrailToolsDetail::DraftPack& pack, int indentTabs)
	{
			...
			!pack.active.type.empty())
		{
			...
			if (!listed)
				EmitTrailTag(os, pack.active, pad);
		}
	}
...
void TrailToolsXml::CoerceSingleOverlayPath(std::wstring& path)
{
	auto ends = [&](const wchar_t* suf) -> bool {
		const size_t n = std::wcslen(suf);
		if (path.size() < n)
			return false;
		return _wcsicmp(path.c_str() + (path.size() - n), suf) == 0;
	};
	if (ends(L"_Menu.xml"))
	{
...
static void DeleteSplitSidecars(const TrailToolsDetail::DraftPack& pack)
{
	DeleteFileW(PackRootXml(pack.packName, L"_Menu.xml").c_str());
	DeleteFileW(PackRootXml(pack.packName, L"_Data.xml").c_str());
}
...
bool TrailToolsXml::WriteOverlayFile(const std::wstring& path, const TrailToolsDetail::DraftPack& pack)
{
	...
	CoerceSingleOverlayPath(out);
	if (!WriteUtf8File(out, EmitOverlayData(pack)))
		return false;
	DeleteSplitSidecars(pack);
	return true;
}
