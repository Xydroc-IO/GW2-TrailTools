...
void TrailToolsUberTool::FollowSelection()
{
	...
	if (gSt.drag != Axis::None)
		return;
	if (gLockPoi >= 0 && gLockPoi < static_cast<int>(gDraft.pois.size()))
	{
		gSt.kind = Kind::Poi;
		gSt.index = gLockPoi;
		gDraft.selectedPoi = gLockPoi;
		return;
	}
	gLockPoi = -1;
...
void TrailToolsUberTool::SelectPoi(int index)
{
	using namespace TrailToolsDetail;
	using namespace TrailToolsUberToolDetail;
	if (index < 0 || index >= static_cast<int>(gDraft.pois.size()))
		return;
	gArmPlane = false;
	gSt.drag = Axis::None;
	gSt.pushed = false;
	gLockPoi = index;
	gDraft.selectedPoi = index;
	RecordingSelectedPoint() = -1;
	gSt.kind = Kind::Poi;
	gSt.index = index;
}

void TrailToolsUberTool::Unlock()
{
	using namespace TrailToolsUberToolDetail;
	gLockPoi = -1;
}

int TrailToolsUberTool::ActivePoiIndex()
{
	using namespace TrailToolsDetail;
	using namespace TrailToolsUberToolDetail;
	const int n = static_cast<int>(gDraft.pois.size());
	if (gLockPoi >= 0 && gLockPoi < n)
		return gLockPoi;
	if (gSt.kind == Kind::Poi && gSt.index >= 0 && gSt.index < n)
		return gSt.index;
	if (gDraft.selectedPoi >= 0 && gDraft.selectedPoi < n)
		return gDraft.selectedPoi;
	return -1;
}
...
bool TrailToolsUberTool::Tick()
{
	...
	if (gSt.drag != Axis::None || gArmPlane)
		return true;
	const ImGuiIO& io = ImGui::GetIO();
	/* Pad hover (Wine) + drain WndProc click armed before ImGui handled it. */
	if (io.WantCaptureMouse || gPadPointerOver)
	{
